...
public class Decryptor implements AESConstants {
	...
	private static void input() throws FileNotFoundException{
		...
		PrintWriter output = new PrintWriter(inputFilename+".dec");
		String line="";
	...
	public static char[][] invSubBytes(char[][] state){
	...
	//Assuming state and roundkey are same size... we need to pass in only the key for
		...
		private static char[][] invMixColumn (char state[][]) {
		...
		// logtables to do the computation.
			...
			state[0][c] = (char)(mul(0xE,a[0]) ^ mul(0xB,a[1]) ^ mul(0xD, a[2]) ^ mul(0x9,a[3]));
			state[1][c] = (char)(mul(0xE,a[1]) ^ mul(0xB,a[2]) ^ mul(0xD, a[3]) ^ mul(0x9,a[0]));
			state[2][c] = (char)(mul(0xE,a[2]) ^ mul(0xB,a[3]) ^ mul(0xD, a[0]) ^ mul(0x9,a[1]));
			state[3][c] = (char)(mul(0xE,a[3]) ^ mul(0xB,a[0]) ^ mul(0xD, a[1]) ^ mul(0x9,a[2]));
			} // mixColumn2
			return state;
			}
		// END DR. YOUNG's CODE///	
